...
{
    /**
     * Liste tous les événements spéciaux
     */
    ...
    /**
     * Crée un nouvel événement spécial
     */
    public function store(Request $request)
    {
        ...
        // Génération automatique de l'ID (format SE000001)
        $lastEvent = SpecialEvent::orderBy('event_id', 'desc')->first();
        $num = $lastEvent ? intval(substr($lastEvent->event_id, 2)) + 1 : 1;
        $newId = 'SE' . str_pad($num, 6, '0', STR_PAD_LEFT);
    ...
    public function update(Request $request, $id)
    {
        ...
        $event->update($request->only([
            'event_type_id', 'event_name', 'event_starting_date', 'event_ending_date', 'event_description'
        ]));
